...
class ThreeLayerConvNet(object):
    """
    A three-layer 7418 network with the following architecture:
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        ## 根据conv_params和pool_params计算W2的第一个维度
    ...
    def loss(self, X, y=None):
        ...
        loss += 0.5 * self.reg * (np.sum(W1**2) + np.sum(W1**2) + np.sum(W1**2))
        
        dout2, grads['W3'], grads['b3'] = affine_backward(dscores, cache)
